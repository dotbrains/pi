...
[![Theme: GitHub Dark](https://img.shields.io/badge/theme-GitHub%20Dark-24292f.svg)](themes/github-dark-default.json)
...
Firecrawl-backed web tools, Git status UI, managed background terminals, and a
GitHub Dark theme. Clone it into `~/.pi/agent`, add your Firecrawl key, and Pi
loads the extensions, skills, and theme on the next start.
...
Svelte-specific skills are intentionally not included. See
[docs/skills.md](docs/skills.md).

## Configuration

Firecrawl tools require `FIRECRAWL_API_KEY` in `~/.pi/agent/.env`. The bundled
theme can be enabled from `~/.pi/agent/settings.json`:
...
backend checks live in `extensions/subagents/` behind `npm run test:live`.
